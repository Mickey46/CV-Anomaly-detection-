...
from tkinter import Canvas, NW, Frame
...
colors = [(255, 255, 255)] * len(camera_indices)  # Default white color for each camera
...
frames = [Frame(root) for _ in camera_indices]
canvases = [Canvas(f, width=512, height=1024) for f in frames]

for f, canvas in zip(frames, canvases):
    canvas.pack()
    f.pack(side=tk.LEFT)

def update_display():
    for idx, (cap, canvas) in enumerate(zip(caps, canvases)):
        ret, frame = cap.read()
        if ret:
            ...
            if prev_frames[idx] is not None:
                ...
                motion_area = np.sum(motion_mask) / (512*512)
                
                # Only update the filter color if significant motion is detected
                if motion_area > 0.2:  # This threshold can be adjusted
                    ...
                    if len(motion_coords) > 0:
                        ...
                        if y < 512:  # Only pick colors from the top half (original image)
                            ...
                            colors[idx] = (int(picked_color[0]), int(picked_color[1]), int(picked_color[2]))
            
            image_batch = np.repeat(frame_resized[np.newaxis, ...], 1, axis=0)
            img_tensor_batch = torch.tensor(image_batch)
            target_color_tensor_batch = torch.tensor(np.array([colors[idx]])).float()
